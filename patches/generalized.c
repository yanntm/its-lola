...
int *fin, *final, scc_size, *bad_scc;
static int scc_id,rank;
    ...
    for (s = gstates->nxt; s != gstates; s = s->nxt)
    {
        ...
        while (t != s->trans)  /* tries to remove t */
        {
            ...
            while ( !((t != t1)
                      ...
                      && (included_set(t->final, t1->final,
                                       0)  /* acceptance conditions of t are also in t1 or may be ignored */
...
int simplify_gstates() /* eliminates redundant states */
{
    ...
    for (a = gstates->nxt; a != gstates; a = a->nxt)
    {
        ...
        b = a->nxt;
        while (!all_gtrans_match(a, b, tl_simp_scc))
        {
            b = b->nxt;
        }
...
GState *find_gstate(int *set, GState *s)
{
    ...
    if (same_sets(set, s->nodes_set, 0))
    {
        return s;    /* same state */
    }
...
void mk_generalized()
{
    ...
    if (init_size)
    {
        init = (GState **)tl_emalloc(init_size * sizeof(GState *));
    }
    init_size = 0;
